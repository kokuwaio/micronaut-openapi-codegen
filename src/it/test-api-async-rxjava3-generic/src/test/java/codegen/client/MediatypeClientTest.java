...
public class MediatypeClientTest implements MediatypeApiTestSpec {
	...
	public void mediatypeConsumesAndProduces200() {
		assertEquals(STRING, assert200(() -> client.mediatypeConsumesAndProduces(BYTES).blockingGet()).body().getFoo());
	}
	...
	public void mediatypeConsumesMultipartWithFileUpload200() {
		...
		assertEquals(expected,
				assert200(() -> client.mediatypeConsumesMultipartWithFileUpload(body).blockingGet()).body());
	}
	...
	public void mediatypeConsumesMultipartWithoutFileUpload200() {
		...
		var body = MultipartBody.builder()
				...
				.build();
		assertEquals(expected,
				assert200(() -> client.mediatypeConsumesMultipartWithoutFileUpload("p", body).blockingGet()).body());
	}

	@Test
	@Override
	public void mediatypeConsumesOctetStream200() {
		assertEquals(STRING, assert200(() -> client.mediatypeConsumesOctetStream(BYTES).blockingGet()).body().getFoo());
	}

	@Test
	@Override
	public void mediatypeConsumesPlain200() {
		assertEquals(STRING, assert200(() -> client.mediatypeConsumesPlain(STRING).blockingGet()).body().getFoo());
	}

	@Test
	@Override
	public void mediatypeProducesSameModel200() {
		assertEquals(STRING, assert200(() -> client.mediatypeProducesSameModel(STRING).blockingGet()).body().getFoo());
	}
