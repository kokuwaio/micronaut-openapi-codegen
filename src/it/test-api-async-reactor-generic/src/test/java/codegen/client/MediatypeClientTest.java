...
public class MediatypeClientTest implements MediatypeApiTestSpec {
	...
	public void mediatypeConsumesAndProduces200() {
		assertEquals(STRING, assert200(() -> client.mediatypeConsumesAndProduces(BYTES).block()).body().getFoo());
	}
	...
	public void mediatypeConsumesMultipartWithFileUpload200() {
		...
		var body = MultipartBody.builder()
				...
				.build();
		assertEquals(expected, assert200(() -> client.mediatypeConsumesMultipartWithFileUpload(body).block()).body());
	}
	...
	public void mediatypeConsumesOctetStream200() {
		assertEquals(STRING, assert200(() -> client.mediatypeConsumesOctetStream(BYTES).block()).body().getFoo());
	}

	@Test
	@Override
	public void mediatypeConsumesPlain200() {
		assertEquals(STRING, assert200(() -> client.mediatypeConsumesPlain(STRING).block()).body().getFoo());
	}

	@Test
	@Override
	public void mediatypeProducesSameModel200() {
		assertEquals(STRING, assert200(() -> client.mediatypeProducesSameModel(STRING).block()).body().getFoo());
	}
