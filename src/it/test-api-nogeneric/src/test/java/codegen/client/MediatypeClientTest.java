...
public class MediatypeClientTest implements MediatypeApiTestSpec {
	...
	public void mediatypeConsumesAndProduces200() {
		assertEquals(STRING, client.mediatypeConsumesAndProduces(BYTES).getFoo());
	}
	...
	public void mediatypeConsumesMultipartWithFileUpload200() {
		...
		var body = MultipartBody.builder()
				...
				.build();
		assertEquals(expected, client.mediatypeConsumesMultipartWithFileUpload(body));
	}
	...
	public void mediatypeConsumesMultipartWithoutFileUpload200() {
		...
		var body = MultipartBody.builder()
				...
				.addPart("userId", String.valueOf(expected.getUserId()))
				.build();
		assertEquals(expected, client.mediatypeConsumesMultipartWithoutFileUpload("p", body));
	}

	@Test
	@Override
	public void mediatypeConsumesOctetStream200() {
		assertEquals(STRING, client.mediatypeConsumesOctetStream(BYTES).getFoo());
	}

	@Test
	@Override
	public void mediatypeConsumesPlain200() {
		assertEquals(STRING, client.mediatypeConsumesPlain(STRING).getFoo());
	}

	@Test
	@Override
	public void mediatypeProducesSameModel200() {
		assertEquals(STRING, client.mediatypeProducesSameModel(STRING).getFoo());
	}
