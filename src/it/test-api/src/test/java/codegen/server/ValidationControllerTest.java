...
public class ValidationControllerTest implements ValidationApiTestSpec {
	...
	public void validationListModel204() {
		assert204(() -> client
				.validationListModel(List.of(new StringModel().foo("foo"), new StringModel().foo("bar"))));
	}
	...
	public void validationListModel400() {
		assert204(() -> client
				.validationListModel(List.of(new StringModel().foo("foo"), new StringModel().foo(null))));
	}

	@Test
	@Override
	public void validationModel204() {
		assert204(() -> client.validationModel(new StringModel().foo("meh")));
	}

	@Test
	@Override
	public void validationModel400() {
		assert400(() -> client.validationModel(new StringModel().foo(null)));
	}
